...
public class Tests {
	...
	public void getStatusFromHomeTimeline() {
		...
		List<StatusFromHomeTimelinePOJO> statuses = given().auth()
				.oauth(consumerAPIKey, consumerAPIsecretKey, accessToken, accessTokenSecret)
				.queryParam("count", getStatusFromHomeTimelineData.getCount()).log().all().when()
				.get(Resources.getStatusFromHomeTimelineResource()).then().log().ifError().extract().response()
				.jsonPath().getList("", StatusFromHomeTimelinePOJO.class);
	...
	public void deleteATweet() {
		...
		datamap = DataHub.DataMap(POSTATWEET, 1);
		PostATweetData postATweetData = new PostATweetData(datamap);
		Response response = given().auth().oauth(consumerAPIKey, consumerAPIsecretKey, accessToken, accessTokenSecret)
				.when().post(Resources.getDeleteTweetResource(id)).then().extract().response();
	...
	public void postWelcomeMessage() {
		...
		Response response = given().auth().oauth(consumerAPIKey, consumerAPIsecretKey, accessToken, accessTokenSecret)
				.header("Content-Type", "application/json").body(getSimpleWelcomePayload(datamap)).when()
				.post(Resources.getWelcomeMessagePostResource()).then().statusCode(200).extract().response();
	...
	public void deleteWelcomeMessage() {
		RestAssured.baseURI = configFileReader.getHost();
		given().auth().oauth(consumerAPIKey, consumerAPIsecretKey, accessToken, accessTokenSecret).queryParam("id", id)
				.when().delete(Resources.getDeleteWelcomeMessageResource()).then().statusCode(204);
	}
